(function (definition) {
    ...
    OOCarousel.SELECTORS = {
        ...
        ITEM_IS_ACTIVE: 'js-oocarousel-item_isActive'
    };
    ...
    OOCarousel.FX = {
        ...
        SLIDE: function (carousel, from, to) {
            ...
            // There's a bug in $.transit where it can't translate a selection containing more than one element
            $items.each(function (i, element) {
                promises.push($(element).transition({ translate: [ to * -100 + '%', 0 ] }, carousel._options.speed, carousel._options.easing));
            });
            return $.when.apply($, promises);
        },

        SLIDE_INFINITE: function (carousel, from, to) {
            ...
            if (carousel._layout !== 'slideInfinite') {
                carousel._layout = 'slideInfinite';
                carousel.$element.css({ position: 'relative' });
                $items.css({ position: 'absolute', top: 0, left: 0, opacity: '', zIndex: 0 });
            }

            return $.when(
                $from.css({ zIndex: 1, translate: [ 0, 0 ] }).transition({ translate: [ 100 * leftOrRight * -1 + '%', 0 ] }),
                $to.css({ zIndex: 1, translate: [ 100 * leftOrRight + '%', 0 ] }).transition({ translate: [ 0, 0 ]})
            );
        },
    ...
    OOCarousel.prototype.layout = function () {
        ...
        this.$element.css({ overflow: 'hidden', whiteSpace: 'nowrap', fontSize: 0 });
        return this;
    };
